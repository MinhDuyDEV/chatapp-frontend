...
interface IMessageEditorProps {
    ...
    setStateEditing: (state: { isEditing: boolean, message: Message | null }) => void;
}

const MessageEditor = ({stateEditing, setStateEditing}: IMessageEditorProps) => {
        ...
        <div className='space-y-2'>
            {isEditing && <div className='flex items-center justify-between'>
                <div className='flex items-start flex-col'>
                    <p className='text-base'>Replying to {message?.author.id === user?.id ? 'yourself' : `${message?.author.username}`}</p>
                    <p className='text-sm text-foreground/70'>{message?.content}</p>
                </div>
                <Button variant='ghost' className='rounded-full' size='iconSm' onClick={() => setStateEditing({ isEditing: false, message: null })}>
                    <X/>
                </Button>
            </div>}
                    ...
                    <div className='absolute right-3 top-1.5 flex items-center z-10'>
                        ...
                        {showEmojiPicker && (
                            <div ref={emojiPickerRef} className='absolute bottom-full right-0 mb-2'>
                                <EmojiPicker emojiStyle={EmojiStyle.FACEBOOK} onEmojiClick={handleEmojiClick}/>
                            </div>
                        )}
                    </div>
